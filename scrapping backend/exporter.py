...
def ensure_all_keys(data_list, column_order):
...
def enrich_and_export(all_filtered, all_manual, LOCATION_KEYWORDS):
    ...
    column_order = ["Message", "Link", "PageContent", "MatchedKeywords"]
    ...
    # Ensure consistency
    filtered_data = ensure_all_keys(filtered_data, column_order)
    manual_data = ensure_all_keys(manual_data, column_order)
    ...
    pd.DataFrame(filtered_data).to_csv(filtered_filename, index=False, encoding="utf-8")
    pd.DataFrame(manual_data).to_csv(manual_filename, index=False, encoding="utf-8")
